          ...
          <div className="space-y-24 animate-fade-in text-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center text-center">
              <div className="space-y-8 text-gray-600 leading-relaxed font-light text-lg text-center md:text-left">
                <div className="flex items-start gap-4 text-[#0a9396] justify-center md:justify-start">
                ...
                <p className="text-center md:text-left">
                  L'idée de KAVAH est née là : créer un vêtement qui soit un <span className="text-[#0a9396] font-medium italic text-center md:text-left">vecteur de transmission</span>.
                </p>
                <p className="text-center md:text-left">
               ...
               <div className="relative perspective-1000 w-full max-w-3xl mx-auto aspect-video md:aspect-[21/9] cursor-pointer group" onClick={() => setIsManifestoFlipped(!isManifestoFlipped)}>
                <div className={`relative w-full h-full transition-all duration-1000 transform-style-3d ${isManifestoFlipped ? 'rotate-y-180 shadow-2xl' : 'shadow-xl'} rounded-[2.5rem]`}>
                  <div className="absolute inset-0 backface-hidden bg-white rounded-[2.5rem] flex flex-col items-center justify-center p-12 border border-gray-100 text-center">
                    <div className="text-8xl md:text-9xl font-serif mb-6 opacity-80 transition-transform group-hover:scale-110 duration-700">כַּוָּנָה</div>
      ...
      {currentPage === 'cart' && (
        <main className="pt-32 pb-20 px-6 md:px-12 max-w-4xl mx-auto min-h-[70vh]">
          <div className="mb-12 text-center md:text-left">
            <h2 className="text-4xl font-light tracking-tight mb-2">Mon Panier</h2>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 animate-fade-in text-center lg:text-left">
              <div className="lg:col-span-2 space-y-6">
                {cart.map((item) => (
                  <div key={item.cartId} className="flex gap-6 p-6 bg-white rounded-3xl border border-gray-50 group hover:shadow-lg transition-all text-center lg:text-left">
                    <div className="w-24 h-32 bg-gray-100 rounded-xl overflow-hidden flex-shrink-0"><img src={userImageUrl} className="w-full h-full object-cover" style={item.views.back} /></div>
                    <div className="flex-grow flex flex-col justify-between py-1">
                      <div className="text-center lg:text-left">
                        <div className="flex justify-between items-start"><h3 className="text-xl font-light">{item.title}</h3><button onClick={() => removeFromCart(item.cartId)} className="text-gray-300 hover:text-red-400 transition-colors"><Trash2 className="w-4 h-4" /></button></div>
                        <div className="flex items-center gap-2 mt-1 justify-center lg:justify-start"><div className="w-2 h-2 rounded-full" style={{ backgroundColor: item.selectedVar.color }}></div><span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{item.selectedVar.name}</span></div>
                      </div>
                      <div className="flex justify-between items-end"><span className="text-sm font-medium text-[#0a9396] italic">Coton Premium</span><span className="text-lg font-bold">{item.price}€</span></div>
                    </div>
                  </div>
                ))}
              </div>
              <div className="lg:col-span-1 h-fit sticky top-32">
                <div className="bg-white p-8 rounded-3xl border border-gray-100 space-y-8 text-center">
                  <h4 className="text-[10px] font-black tracking-widest uppercase text-gray-400">Résumé</h4>
                  <div className="space-y-4">
                    <div className="flex justify-between text-sm"><span className="text-gray-400 text-center">Sous-total</span><span>{cartTotal} €</span></div>
                    <div className="flex justify-between text-sm"><span className="text-gray-400 text-center">Livraison</span><span className="text-[#0a9396] font-bold uppercase text-[10px]">Offerte</span></div>
                    <div className="h-px bg-gray-50 pt-4"></div>
                    <div className="flex justify-between text-xl font-light"><span>Total</span><span>{cartTotal} €</span></div>
                  </div>
                  <button className="w-full bg-[#1a1a1a] text-white py-5 rounded-xl text-xs font-bold tracking-widest uppercase hover:bg-[#0a9396] transition-all shadow-xl">Paiement Sécurisé</button>
                </div>
              </div>
            </div>
          )}

  ...
  return (
    <div className="min-h-screen bg-[#FDFCF8] text-[#1a1a1a] font-sans overflow-x-hidden">
          ...
          {viewType === 'grid' ? (
            ...
            <div className="relative min-h-[70vh] flex items-center animate-fade-in">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 items-center w-full">
                ...
                <div className="relative group perspective-1000 mx-auto w-full max-w-md text-center">
      ...
      {currentPage === 'histoire' && (
        <main className="pt-32 pb-32 px-6 md:px-12 max-w-5xl mx-auto text-center">
          <div className="space-y-24 animate-fade-in text-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center text-center">
              <div className="space-y-8 text-gray-600 leading-relaxed font-light text-lg text-center md:text-left">
                ...
                <p className="text-center md:text-left">
                  Nous nous sommes demandé comment transmettre cette étincelle de manière moderne. L'idée de KAVAH est née là : créer un vêtement qui soit un <span className="text-[#0a9396] font-medium italic text-center md:text-left">vecteur de transmission</span>.
                </p>
                <p className="text-center md:text-left">
                  C'est pour cette raison que chaque T-shirt KAVAH est accompagné d'une <span className="text-[#1a1a1a] font-bold">carte de sagesse</span> offrant une info-bulle sur la fête ou la valeur qu'il incarne.
                </p>
              </div>
            ...
            <div className="space-y-12 text-center">
               <div className="text-center space-y-2">
                  <h3 className="text-[10px] font-black tracking-[0.5em] uppercase text-gray-300">Au-delà du vêtement</h3>
                  <p className="text-2xl font-light">Comprendre le mot qui nous définit</p>
               </div>
               ...
               <div className="relative perspective-1000 w-full max-w-3xl mx-auto aspect-video md:aspect-[21/9] cursor-pointer group" onClick={() => setIsManifestoFlipped(!isManifestoFlipped)}>
                <div className={`relative w-full h-full transition-all duration-1000 transform-style-3d ${isManifestoFlipped ? 'rotate-y-180 shadow-2xl' : 'shadow-xl'} rounded-[2.5rem]`}>
                  ...
                  <div className="absolute inset-0 backface-hidden bg-white rounded-[2.5rem] flex flex-col items-center justify-center p-12 border border-gray-100 text-center">
                    ...
                    <p className="text-gray-300 text-[8px] tracking-[0.3em] uppercase flex items-center gap-2 justify-center">
                  ...
                  <div className="absolute inset-0 backface-hidden bg-[#1a1a1a] rounded-[2.5rem] flex flex-col items-center justify-center p-12 transform rotate-y-180 text-white overflow-hidden text-center">
                    ...
                    <div className="max-w-xl space-y-6 text-center leading-relaxed font-light text-sm md:text-base mx-auto">
                      <p>
                        La <span className="font-bold italic">Kavanah</span> vient de la racine hébraïque signifiant "diriger" ou "viser". C'est l'orientation de l'esprit et du cœur vers un but unique.
                      </p>
                      <p>
                        Dans la tradition, on enseigne qu'une action sans Kavanah est comme "un corps sans âme". Ce n'est pas ce que vous faites qui compte le plus, mais la <span className="text-[#0a9396] font-medium">conscience</span> que vous y mettez.
                      </p>
                    </div>
                  </div>
                </div>
      ...
      {currentPage === 'cart' && (
        <main className="pt-32 pb-20 px-6 md:px-12 max-w-4xl mx-auto min-h-[70vh]">
          <div className="mb-12 text-center md:text-left text-center md:text-left">
              ...
              <div className="lg:col-span-2 space-y-6 text-center lg:text-left">
                {cart.map((item) => (
                  <div key={item.cartId} className="flex gap-6 p-6 bg-white rounded-3xl border border-gray-50 group hover:shadow-lg transition-all text-center lg:text-left">
                    <div className="w-24 h-32 bg-gray-100 rounded-xl overflow-hidden flex-shrink-0 text-center"><img src={userImageUrl} className="w-full h-full object-cover" style={item.views.back} /></div>
                    <div className="flex-grow flex flex-col justify-between py-1 text-center lg:text-left">
                      <div className="text-center lg:text-left">
                        <div className="flex justify-between items-start text-center lg:text-left"><h3 className="text-xl font-light text-center lg:text-left">{item.title}</h3><button onClick={() => removeFromCart(item.cartId)} className="text-gray-300 hover:text-red-400 transition-colors text-center lg:text-left"><Trash2 className="w-4 h-4" /></button></div>
                        <div className="flex items-center gap-2 mt-1 justify-center lg:justify-start text-center lg:text-left"><div className="w-2 h-2 rounded-full" style={{ backgroundColor: item.selectedVar.color }}></div><span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest text-center lg:text-left">{item.selectedVar.name}</span></div>
                      </div>
                      <div className="flex justify-between items-end text-center lg:text-left"><span className="text-sm font-medium text-[#0a9396] italic text-center lg:text-left">Coton Premium</span><span className="text-lg font-bold text-center lg:text-left">{item.price}€</span></div>
                    </div>
                  </div>
                ))}
              </div>
              <div className="lg:col-span-1 h-fit sticky top-32 text-center">
      ...
      {currentPage === 'product' && currentProduct && (
        <main className="pt-32 pb-20 px-6 md:px-12 max-w-7xl mx-auto text-center lg:text-left">
          <button onClick={() => { setCurrentPage('collections'); setViewType('immersive'); }} className="flex items-center gap-2 text-gray-400 text-[10px] font-bold tracking-widest uppercase mb-12 hover:text-[#1a1a1a] transition-all text-center lg:text-left"><ArrowLeft className="w-4 h-4" /> Retour Boutique</button>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start animate-fade-in text-center lg:text-left">
            <div className="space-y-6 sticky top-32 mx-auto">
              <div className="relative aspect-[4/5] bg-[#f5f5f5] rounded-[2rem] overflow-hidden shadow-2xl transition-all duration-700 mx-auto">
                <img src={userImageUrl} alt={currentProduct.title} className="w-full h-full object-cover transition-all duration-1000 ease-in-out" style={{ objectPosition: currentProduct.views[viewSide].objectPosition, transform: `scale(${currentProduct.views[viewSide].scale})` }}/>
                <div className="absolute top-6 left-8 bg-white/90 px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest text-[#0a9396] text-center">{viewSide === 'back' ? "DOS" : "AVANT"}</div>
              </div>
              <button onClick={() => setViewSide(viewSide === 'back' ? 'front' : 'back')} className="w-full bg-[#1a1a1a] text-white py-6 rounded-xl text-[10px] font-bold uppercase tracking-widest flex items-center justify-center gap-3 hover:bg-[#0a9396] transition-all shadow-lg text-center"><RefreshCw className={`w-4 h-4 transition-transform duration-700 ${viewSide === 'front' ? 'rotate-180' : ''}`} /> {viewSide === 'back' ? "Découvrir la Signature" : "Revenir au Design Arrière"}</button>
            </div>
            
            <div className="space-y-12 text-center lg:text-left">
              <div className="space-y-4 text-center lg:text-left">
                <h1 className="text-7xl md:text-[10rem] font-light tracking-tighter leading-none text-center lg:text-left">{currentProduct.title}</h1>
                <p className="text-xl text-gray-500 font-light leading-relaxed max-w-md italic mx-auto lg:mx-0 text-center lg:text-left">{currentProduct.description}</p>
              </div>

              <div className="bg-white rounded-[2rem] border border-gray-100 shadow-sm overflow-hidden transition-all duration-500 text-center lg:text-left">
                <button onClick={() => setIsConfigExpanded(!isConfigExpanded)} className="w-full px-8 py-6 flex justify-between items-center hover:bg-gray-50 transition-colors text-center lg:text-left">
                  <span className="text-[10px] font-black tracking-[0.3em] uppercase text-gray-400 text-center lg:text-left">Personnaliser l'Intention</span>
                  {isConfigExpanded ? <Minus className="w-4 h-4 text-gray-400" /> : <Plus className="w-4 h-4 text-[#0a9396]" />}
                </button>
                {isConfigExpanded && (
                  <div className="px-8 pb-8 space-y-8 animate-fade-in text-center lg:text-left">
                    <div className="flex flex-wrap gap-6 pt-4 justify-center lg:justify-start">
                      {currentProduct.variables.map((v, idx) => (
                        <button key={idx} onClick={() => { setSelectedVariable(idx); setViewSide('front'); }} className={`relative w-14 h-14 rounded-full border-2 transition-all duration-500 hover:scale-110 active:scale-95 ${selectedVariable === idx ? 'border-[#1a1a1a] scale-110 shadow-lg' : 'border-transparent opacity-30 grayscale hover:opacity-100'}`} style={{ backgroundColor: v.color }} />
                      ))}
                    </div>
                    <div className="text-[10px] text-gray-400 font-bold uppercase tracking-widest transition-all duration-500 text-center lg:text-left" style={{ color: currentProduct.variables[selectedVariable].color }}>Signature : <span className="font-black underline text-center lg:text-left">{currentProduct.variables[selectedVariable].name}</span></div>
                  </div>
                )}
              </div>

              <div className="space-y-4 text-center lg:text-left">
                <button onClick={addToCart} className="w-full bg-[#1a1a1a] text-white py-7 rounded-sm text-xs font-bold tracking-[0.4em] uppercase hover:bg-[#0a9396] transition-all shadow-2xl transform hover:-translate-y-1 text-center">Ajouter au panier — 55€</button>
              </div>
            </div>
          </div>
        </main>
      ...
      <footer className="py-24 px-6 border-t border-gray-100 bg-white text-center">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-12 text-center">
          ...
          <div className="flex gap-12 text-[10px] font-bold tracking-widest text-gray-400 uppercase text-center justify-center">
            <a href="https://instagram.com" target="_blank" rel="noopener noreferrer" className="hover:text-[#0a9396] text-center">Instagram</a>
            <a href="https://tiktok.com" target="_blank" rel="noopener noreferrer" className="hover:text-[#0a9396] text-center">TikTok</a>
            <a href="mailto:contact@kavah.com" className="hover:text-[#0a9396] text-center">Contact</a>
          </div>
